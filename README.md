...
> Status: **v0.10.4.** Critical GUI fixes for VS Code webview: persistent profile delete + Gemini save UX + per-profile enable toggle + auto-discover button — `confirm()` is blocked in VS Code webviews, which silently aborted every delete; now uses a safeConfirm fallback that proceeds when the native dialog is unavailable. Also wires SecretsStore in the VS Code extension's daemon (was missing — `/v1/secrets` returned 501 so API key paste never worked). v0.10.3: addProfile no longer crashes on null entries from prior deletes. v0.10.2: chat XML leak fix + Mistral `[TOOL_CALLS]` format. Adds LLM-generated plugins (describe a plugin in natural language → Tierkit auto-routes the description through an LLM → previews a manifest + rule files → one-click install), model management UI (add/delete profiles, paste API keys from the GUI), Gemini preset, cross-tier auto-fallback (local → private-remote → public-cloud), response quality evaluator (retries on refusal/empty/truncated/repetition), task-type classifier + per-profile `goodAt[]`, and budget-aware downgrade. Prior: tool-shim for weak local models, plugin-rule system-prompt injection, profile viability pre-flighting, mission-control sidebar, OpenAI-compatible endpoint for Roo/Cline/Continue/aider/etc. See the [roadmap in SPEC.md §15](docs/SPEC.md#15-roadmap).
