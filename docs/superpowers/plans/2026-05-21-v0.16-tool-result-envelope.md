...
### Task 1.0: Pre-flight — verify import path convention against actual repo

Before starting any sub-task, scan this plan's code blocks for relative imports and verify they match the existing codebase. The repo convention (confirmed against `packages/agent/src/tools/readFile.ts`):

- **Test files** at `packages/*/test/X.test.ts` import source via `../src/tools/Y.js` ✓
- **Source files** at `packages/*/src/tools/X.ts` import siblings via `./Y.js` and parent dir via `../types.js` ✓
- Never `from "./src/..."` inside a test file or `from "./types.js"` inside a `src/tools/` file.

If you find any plan code with the wrong form (`./src/...` from a test, or `./types.js` from `src/tools/`), STOP and surface it as a plan bug before writing code. The current plan is verified clean — this check is a guard against drift if you copy-edit any task.
...
describe("runChild shell-gate by platform", () => {
  ...
  beforeEach(() => {
    // Stub spawn so we can read its call args without actually launching a process.
...
Preserve the v0.15 gating (SKIP_DIRS, dotfile skip, stat-then-readdir error branching). Wrap the final entry list in an envelope and add pagination via `nextIndex`. Full rewrite:
...
export const listFilesTool: Tool = {
  ...
  async execute(args: Record<string, unknown>, ctx: AgentContext): Promise<ToolResult> {
    ...
    try { stat = await fs.stat(rootAbs); }
      ...
      return envelopeToString(makeFailureEnvelope("list_files", "read-error", e.message));
    }
    if (!stat.isDirectory()) {
...
- [ ] **Step 4: Run → PASS (with caveat that summarizeOutput is still v0.15 shape)**

```bash
pnpm -F @tierkit/mcp-server build
pnpm -F @tierkit/mcp-server exec vitest run test/runCommandEnvelope.test.ts
```

The package-wide test run will still pass at this point because `summarizeOutput` reads non-existent fields and returns undefined or wrong values — no test asserts on the activity-log summary shape yet. The summary is fixed in Task 4.5.

- [ ] **Step 5: Commit**

```bash
git add packages/mcp-server/src/tools/runCommand.ts packages/mcp-server/test/runCommandEnvelope.test.ts
git commit -m "feat(mcp-server, v0.16): tierkit.run_command returns envelope with stream-prefixed size"
```

---

### Task 4.5: Update MCP `server.ts` summarizeInput/Output for envelope shape (all 4 tools)

**Files:**
- Modify: `packages/mcp-server/src/server.ts`
- Modify: `packages/mcp-server/test/mcpServer.test.ts` (activity-log assertion)

Tasks 4.1–4.4 changed every long-output MCP tool's result shape from `{ ok, content, files, matches, ... }` to `{ ok, tool, version, data: {...}, truncated, ... }`. The existing `summarizeOutput()` in `server.ts` still reads top-level fields like `result.content` / `result.files` / `result.matches` / `result.exitCode` — those are now buried under `result.data`. The activity log entries for v0.16 tool calls show `null` / `undefined` summaries until this is fixed.

Also: introduce a new `summarizeEnvelope(result)` helper that extracts the v0.16 `envelope.{truncated,hasCursor,remainingLines}` triple for the activity log (consumed by Task 5.2).

- [ ] **Step 1: Update `summarizeOutput` to read from `result.data.*` for the 4 envelope tools**

In `packages/mcp-server/src/server.ts`, find `summarizeOutput()` and replace its 4 envelope-tool branches:
...
function summarizeOutput(name: ToolName, result: ToolEnvelope): unknown {
  ...
  if (name === "tierkit.get_policy_status") {
    return { pendingPatches: (result as any).pendingPatches };
  }
  ...
  switch (name) {
    ...
    case "tierkit.propose_patch":
      return { patchId: (result as any).patchId, riskLevel: ((result as any).risk as any)?.level };
    case "tierkit.apply_patch":
      return { fileCount: Array.isArray((result as any).files) ? (result as any).files.length : 0 };
    default:
      return null;
  }
}
...
`propose_patch` and `apply_patch` are NOT migrated to the envelope (v0.16 spec §B.5 — out of scope), so their branches still read top-level fields.

- [ ] **Step 2: Add `summarizeEnvelope()` helper**

In the same file, add near the other helpers:
...
- [ ] **Step 3: Wire `summarizeEnvelope()` into `withActivityLog`**

Find the `withActivityLog` function and update the `logMcpActivity` call:
...
await logMcpActivity(ctx.workspaceRoot, {
  ...
  envelope: summarizeEnvelope(result),   // NEW in v0.16
}).catch(() => { /* swallow */ });
```

Note: the `envelope` field on `McpActivityInput` is added in Task 5.2; this task assumes that interface change is already in place. If executing in strict phase order, Task 5.2 lands the interface change first OR this task lands its server.ts edit with a `// @ts-expect-error: envelope field added in Task 5.2` comment that gets removed once 5.2 ships. The cleanest path is to merge Task 5.2's `McpActivityInput.envelope` change first, then this Task 4.5 update — adjust the execution order if you're doing them as separate commits.

- [ ] **Step 4: Add activity-log assertion to mcpServer test**

Append to `packages/mcp-server/test/mcpServer.test.ts` (or wherever the existing smoke test lives):
...
(The `callOneTool` helper is whatever was established in v0.15's Task 1.4 — InMemoryTransport or subprocess driver.)
...
git commit -m "feat(mcp-server, v0.16): summarizeOutput reads result.data + activity log envelope field"
```
...
| §B.4 per-tool definitions (MCP) | Tasks 4.1, 4.2, 4.3, 4.4 + 4.5 (server.ts summarizeOutput + activity envelope wiring) |
