...
- **0.25.1** — Gateway sidebar status fix. Status refresh now falls back from stuck `Loading...` to an explicit unavailable state, and the sidebar shows read-only Measured Compact Context status when the daemon reports it. No official token reduction number is displayed without real request-level Anthropic Token Counting API measurement.
